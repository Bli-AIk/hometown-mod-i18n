...
if Game.battle.encounter.id == "forced" then 
    ...
    for _, child in ipairs(Game.battle.children) do 
        if child:includes(ActionButton) then 
            child:remove()
        end 
    end   
    ...
    function()
        ...
        if not (Game.battle.turn_count == 4) then
        self:increaseStat("magic", -5)   
        end      
        local ralsei = Game.battle:getEnemyBattler("ralsei_forced")
        local shield = ShieldEffect(ralsei, 30, 1)
        shield.x = (ralsei.width / 2) - 7
        shield.y = (ralsei.height / 2) + 5 
        ralsei:setAnimation("spell")
        ralsei:addChild(shield)
        end 
        local ralsei = Game.battle:getEnemyBattler("ralsei_forced")
        if not (Game.battle.turn_count > 2) then
