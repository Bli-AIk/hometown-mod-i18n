-- Define the table up here so both leaf_spawn and fall can read/write to it
local all_leaves = {}
local pit_void = nil
...
return {
    ...
    kris3 = function(cutscene)
        ...
        cutscene:text("* Ralsei,[wait:5] you've always been so attached to Kris...")
        cutscene:setTextboxTop(false)
        cutscene:text("* W-[wait:2]what? Who said that?", "disappointed_side", "ralsei")
        Game.world.music:fade(0, 2)
        cutscene:wait(2)
        cutscene:setTextboxTop(true)
        cutscene:text("[shake:1]* Your worst nightmare.")
        cutscene:detachCamera()
        cutscene:detachFollowers()
        ...
        cutscene:wait(2.5)
        cutscene:setTextboxTop(false)
        cutscene:text("* If you think you're intimidating...[wait:5]", "determined_up", "ralsei")
        cutscene:text("* I've met worse than whoever you think you are!", "angry", "ralsei")
        cutscene:text("* And if you're troubling Kris,[wait:5] then...", "no_glasses_closed", "ralsei")
        Assets.playSound("weaponpull_fast")
        ralsei:setAnimation("battle/attack_ready")
        cutscene:wait(0.5)
        cutscene:text("* Bring it on.", "determined", "ralsei")
        cutscene:setTextboxTop(true)
        cutscene:text("* Always been the gullible type,[wait:5] haven't you?")
...
shadow:setPosition(target_x, target_y)
        ...
        cutscene:text("* Oh Ralsei...[wait:5] always worrying about the details.")
        ralsei:setFacing("right")
        cutscene:wait(0.5)
        cutscene:text("* I'll leave Kris alone...[wait:5]")
        cutscene:wait(0.5)
        cutscene:text("[shake:1]* As soon as you die.")
        cutscene:setTextboxTop(false)
        cutscene:text("* I don't remember that being in the prophecy.", "determined_up", "ralsei")
