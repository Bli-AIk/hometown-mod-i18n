...
kris:setFacing("left")
    ...
    ralsei = function(cutscene)
        ...
        local kris = cutscene:getCharacter("kris")
        cutscene:wait(cutscene:walkTo(kris, rx, ry, 1))
        cutscene:panTo(Game.world.camera.x + 200, Game.world.camera.y, 1, "in-quad")
        cutscene:wait(1)
        local ralsei = Game.world:spawnNPC("ralsei", 948, kris.y + 6)
        ralsei:setFacing("left")
        ralsei:walkTo(730, ralsei.y, 0.6)
        cutscene:wait(0.7)
        ...
        cutscene:text("[speed:1.2][noskip]* Kris,[wait:2] where do we go now-[next]", "small_smile")
        ralsei:resetPhysics()
