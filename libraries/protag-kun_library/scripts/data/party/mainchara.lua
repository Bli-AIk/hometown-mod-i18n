local character, super = Class(PartyMember, "mainchara")
...
function character:init()
    ...
    self:setActor("mainchara")
    self:setLightActor("mainchara_lw")
    self:setDarkTransitionActor("mainchara_dark_transition")
    ...
    self.dmg_color = {1, 0.75, 0.75}
    -- Attack bar color (for the target bar used in attack mode) (defaults to the main color)
    self.attack_bar_color = {1, 0, 0}
    -- Attack box color (for the attack area in attack mode) (defaults to darkened main color)
    self.attack_box_color = {0.5, 0, 0}
    -- X-Action color (for the color of X-Action menu items) (defaults to the main color)
    self.xact_color = {1, 0.5, 0.5}

    -- Head icon in the equip / power menu
    self.menu_icon = "party/mainchara/head"
    -- Path to head icons used in battle
    self.head_icons = "party/mainchara/icon"
    -- Name sprite
    self.name_sprite = nil

    -- Effect shown above enemy after attacking it
    self.attack_sprite = "effects/attack/cut_mc"
