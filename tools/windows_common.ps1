...
function Get-TMSharedToolsDir {
    ...
    if ($env:HOMETOWN_MOD_I18N_TOOLS_DIR) {
        return [System.IO.Path]::GetFullPath($env:HOMETOWN_MOD_I18N_TOOLS_DIR)
    }
    ...
    foreach ($engine in @($env:HOMETOWN_MOD_I18N_KRISTAL_DIR, $env:KRISTAL_ROOT)) {
        if (-not $engine) {
            continue
        }
...
function Invoke-TMBuildHelper {
    ...
    $previousArgumentsFile = $env:HOMETOWN_MOD_I18N_HELPER_ARGS
    try {
        $encoding = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($argumentsFile, (($Arguments -join "`n") + "`n"), $encoding)
        $env:HOMETOWN_MOD_I18N_HELPER_ARGS = $argumentsFile
        ...
        if ($null -eq $previousArgumentsFile) {
            Remove-Item Env:HOMETOWN_MOD_I18N_HELPER_ARGS -ErrorAction SilentlyContinue
        } else {
            $env:HOMETOWN_MOD_I18N_HELPER_ARGS = $previousArgumentsFile
        }
    }
}
...
function Open-TMOutputDirectory {
    ...
    if ($env:HOMETOWN_MOD_I18N_NO_OPEN_DIR -eq '1') {
        return
    }
