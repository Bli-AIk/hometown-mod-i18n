...
$BuildRoot = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_BUILD_ROOT' (Join-Path $Root '.build\standalone')
...
$ModId = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_MOD_ID' 'hometown-mod-i18n'
$ProjectTitle = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_PROJECT_TITLE' 'Hometown Mod'
$OutputBasename = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_OUTPUT_BASENAME' 'hometown-mod-i18n'
$ExeBasename = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_EXE_BASENAME' 'HOMETOWN-MOD-I18N'
$LoveVersion = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_LOVE_VERSION' '11.5'
$LoveArch = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_LOVE_ARCH' 'win64'
$script:TMGit = $null
...
function Find-TMLocalKristal {
    ...
    foreach ($configured in @($env:HOMETOWN_MOD_I18N_KRISTAL_DIR, $env:KRISTAL_ROOT)) {
...
function Resolve-TMKristal {
    ...
    $configuredReference = $env:HOMETOWN_MOD_I18N_KRISTAL_REF
    $reference = if ($configuredReference) { $configuredReference } else { $pinnedReference }
    $source = $env:HOMETOWN_MOD_I18N_KRISTAL_SOURCE
    $configuredDirectory = if ($env:HOMETOWN_MOD_I18N_KRISTAL_DIR) {
        $env:HOMETOWN_MOD_I18N_KRISTAL_DIR
    ...
    if ($source -notin @('local', 'path', 'commit', 'tag', 'branch')) {
        throw "Unknown HOMETOWN_MOD_I18N_KRISTAL_SOURCE: $source"
    }
    if ($source -eq 'commit' -and $reference -notmatch '^[0-9a-fA-F]{40}$') {
        throw 'HOMETOWN_MOD_I18N_KRISTAL_SOURCE=commit requires a 40-character HOMETOWN_MOD_I18N_KRISTAL_REF.'
    }
    if ($source -in @('tag', 'branch') -and [string]::IsNullOrWhiteSpace($reference)) {
        throw "HOMETOWN_MOD_I18N_KRISTAL_SOURCE=$source requires HOMETOWN_MOD_I18N_KRISTAL_REF."
    }
    ...
    if ($source -in @('local', 'path')) {
        ...
        if (-not $directory -or -not (Test-Path -LiteralPath (Join-Path $directory 'main.lua'))) {
            throw 'No usable local Kristal checkout was found. Set HOMETOWN_MOD_I18N_KRISTAL_DIR or use the default pinned source.'
        }
        ...
        if (Test-Path -LiteralPath $directory) {
            ...
            Invoke-TMNative $git @('-C', $directory, 'remote', 'add', 'origin', (Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_KRISTAL_REPO' 'https://github.com/KristalTeam/Kristal.git'))
        }
        ...
        } elseif ($env:HOMETOWN_MOD_I18N_UPDATE_REPOS -eq '1') {
    ...
    $expectedVersion = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_KRISTAL_EXPECTED_VERSION' '0.11.0-dev'
    $verifyVersion = if ($null -ne $env:HOMETOWN_MOD_I18N_KRISTAL_VERIFY_VERSION) {
        $env:HOMETOWN_MOD_I18N_KRISTAL_VERIFY_VERSION -eq '1'
    } else {
...
function Stage-TMWindowIcon {
    ...
    $iconDirectory = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ICON_DIR' (Join-Path $Root 'assets\icon')
    $windowIcon = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_WINDOW_ICON' (Join-Path $iconDirectory 'window_icon.png')
...
function Prepare-TMStandaloneStage {
    ...
    if ($env:HOMETOWN_MOD_I18N_ANDROID_TOUCH_SKIP_INTRO -eq '1') {
        Invoke-TMBuildHelper $Root $LoveExecutable @(
...
function Ensure-TMWindowsLoveDistribution {
    $url = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_LOVE_WINDOWS_ZIP_URL' "https://github.com/love2d/love/releases/download/$LoveVersion/love-$LoveVersion-$LoveArch.zip"
...
function Get-TMWindowsIcon {
    ...
    $iconDirectory = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ICON_DIR' (Join-Path $Root 'assets\icon')
    $windowsIconDirectory = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_WIN_ICON_DIR' (Join-Path $iconDirectory 'win')
...
function Get-TMRcedit {
    $configured = $env:HOMETOWN_MOD_I18N_RCEDit
    if ($configured) {
    ...
    if ($command) { return $command.Source }
    if ($env:HOMETOWN_MOD_I18N_ICON_FETCH_TOOLS -ne '1') { return $null }
    $url = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_RCEDit_URL' 'https://github.com/electron/rcedit/releases/download/v2.0.0/rcedit-x64.exe'
...
function Build-TMModPackage {
    ...
    $buildDirectory = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_MOD_BUILD_DIR' (Join-Path $Root '.build\mod')
    $stageDirectory = Join-Path $buildDirectory 'source'
    $outputFile = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_MOD_OUTPUT_FILE' (Join-Path $OutputDir 'hometown-mod-i18n-mod.zip')
    Remove-TMPathIfPresent $stageDirectory
    New-Item -ItemType Directory -Force -Path $stageDirectory | Out-Null
...
$variants = (Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_BUILD_VARIANTS' 'release debug').Split(@(' ', "`t"), [System.StringSplitOptions]::RemoveEmptyEntries)
if ($variants.Count -eq 0) {
    throw 'HOMETOWN_MOD_I18N_BUILD_VARIANTS must name at least one variant.'
}
