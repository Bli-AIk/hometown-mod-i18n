...
# explicit KRISTAL_ROOT/HOMETOWN_MOD_I18N_KRISTAL_DIR → nearest engine by walking
# up from the mod root → fall back to the mod root.
$KristalRoot = $null
foreach ($candidate in @($env:KRISTAL_ROOT, $env:HOMETOWN_MOD_I18N_KRISTAL_DIR)) {
...
function Get-GitBash {
    ...
    $ver = if ($env:HOMETOWN_MOD_I18N_GIT_VERSION) { $env:HOMETOWN_MOD_I18N_GIT_VERSION } else { '2.55.0.3' }
...
function Get-JavaHome {
    ...
    if ($env:HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME -and
        (Test-Path (Join-Path $env:HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME 'bin\java.exe'))) {
        return $env:HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME
    }
...
if ($Mode -eq 'menu') {
    Write-Host ''
    Write-Host 'Hometown Mod - Android 打包'
...
$env:HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME = $javaHome
$env:LOVE = $loveExe.Replace('\', '/')
$loveDir = Split-Path -Parent $loveExe
if (-not $env:PATH.Contains($loveDir)) { $env:PATH = "$loveDir;$env:PATH" }
$env:HOMETOWN_MOD_I18N_KRISTAL_SOURCE = 'commit'
$env:HOMETOWN_MOD_I18N_KRISTAL_REF = 'f62afea63ccab02f468c24ac0d096bd8a2c9aa81'
$env:HOMETOWN_MOD_I18N_KRISTAL_EXPECTED_VERSION = '0.11.0-dev'
