...
$OutputDir = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_OUTPUT_DIR' (Join-Path $Root 'dist')
$OutputBasename = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_OUTPUT_BASENAME' 'hometown-mod-i18n'
$AndroidBuildToolsVersion = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_BUILD_TOOLS_VERSION' '34.0.0'
$AndroidWorkDir = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_WORK_DIR' (Join-Path $Root '.build\android')
$AndroidWrapWorkDir = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_WRAP_WORK_DIR' (Join-Path $Root '.build\android-wrap')
$CacheDir = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_CACHE_DIR' (Join-Path $Root '.build\cache')
...
function Get-TMJavaHome {
    foreach ($configured in @($env:HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME, $env:JAVA_HOME)) {
    ...
    if ($env:HOMETOWN_MOD_I18N_FETCH_JDK -eq '0') {
        throw 'No JDK 17 is available and HOMETOWN_MOD_I18N_FETCH_JDK=0.'
    }
...
function Get-TMAndroidSdk {
    ...
    if ($env:HOMETOWN_MOD_I18N_FETCH_SDK -eq '0') {
        throw 'A complete Android SDK is required and HOMETOWN_MOD_I18N_FETCH_SDK=0.'
    }
...
function Get-TMAndroidBuildTools {
    $candidates = @($env:HOMETOWN_MOD_I18N_ANDROID_BUILD_TOOLS_DIR)
...
function Build-TMAndroidLove {
    ...
    $saved = @{}
    foreach ($name in @('LOVE', 'HOMETOWN_MOD_I18N_ANDROID_TOUCH_SKIP_INTRO', 'HOMETOWN_MOD_I18N_BUILD_VARIANTS', 'HOMETOWN_MOD_I18N_OUTPUT_DIR', 'HOMETOWN_MOD_I18N_NO_OPEN_DIR')) {
        $saved[$name] = [Environment]::GetEnvironmentVariable($name, 'Process')
    }
    try {
        $env:LOVE = $LoveExecutable
        $env:HOMETOWN_MOD_I18N_ANDROID_TOUCH_SKIP_INTRO = '1'
        $env:HOMETOWN_MOD_I18N_BUILD_VARIANTS = 'release'
        $env:HOMETOWN_MOD_I18N_OUTPUT_DIR = $loveDirectory
        $env:HOMETOWN_MOD_I18N_NO_OPEN_DIR = '1'
...
function Get-TMEmbedApk {
    $explicit = $env:HOMETOWN_MOD_I18N_ANDROID_EMBED_APK
    if ($explicit) {
        if (-not (Test-Path -LiteralPath $explicit)) { throw "Embed APK does not exist: $explicit" }
        return [System.IO.Path]::GetFullPath($explicit)
    }
    $url = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_EMBED_APK_URL' 'https://github.com/love2d/love-android/releases/download/11.5a/love-11.5-android-embed.apk'
    $apk = Join-Path $CacheDir (Split-Path -Leaf $url)
    if (-not (Test-Path -LiteralPath $apk)) {
        Invoke-TMDownload $url $apk
    }
    $expected = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_EMBED_APK_SHA256' 'dcf71c1b54c5b5a09598ef1e6cf4852ced5e5e612de3d0f30cfdd39b5014e889'
    if ($expected) {
...
function Get-TMAndroidSigning {
    ...
    if ($env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE) {
        foreach ($name in @(
            'HOMETOWN_MOD_I18N_ANDROID_SIGNING_STORE_PASSWORD',
            'HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_ALIAS',
            'HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_PASSWORD'
        )) {
            if (-not [Environment]::GetEnvironmentVariable($name, 'Process')) {
                throw "$name is required with HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE."
            }
        }
        if (-not (Test-Path -LiteralPath $env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE)) {
            throw "Android signing keystore does not exist: $env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE"
        }
        return [PSCustomObject]@{
            Path = [System.IO.Path]::GetFullPath($env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE)
            StorePassword = $env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_STORE_PASSWORD
            Alias = $env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_ALIAS
            KeyPassword = $env:HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_PASSWORD
        }
    }
...
function Get-TMLoveAndroidSource {
    $git = Get-TMAndroidGit
    $repository = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_REPO' 'https://github.com/love2d/love-android.git'
    $reference = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_REF' '11.5'
    $cache = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_CACHE_DIR' (Join-Path $CacheDir 'love-android-11.5')
...
function Stage-TMAndroidIcons {
    ...
    $iconDirectory = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_ICON_DIR' (Join-Path $Root 'assets\icon\android')
    $singleIcon = $env:HOMETOWN_MOD_I18N_ANDROID_ICON
...
function Get-TMAndroidCompileSettings {
    $applicationId = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_APPLICATION_ID' 'org.hometownmodi18n.template'
    $name = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_NAME' 'Hometown Mod'
    $orientation = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_ORIENTATION' 'landscape'
    $versionCode = Get-TMEnvOrDefault 'HOMETOWN_MOD_I18N_ANDROID_VERSION_CODE' '1'
    $versionName = if ($env:HOMETOWN_MOD_I18N_ANDROID_VERSION_NAME) {
        $env:HOMETOWN_MOD_I18N_ANDROID_VERSION_NAME
    } else {
        Get-TMModVersion
    }
...
if ($Mode -eq 'menu') {
    Write-Host ''
    Write-Host 'Hometown Mod Android packaging'
