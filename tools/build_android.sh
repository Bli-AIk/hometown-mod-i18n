...
HOMETOWN_MOD_I18N_ANDROID_APPLICATION_ID="${HOMETOWN_MOD_I18N_ANDROID_APPLICATION_ID:-org.hometownmodi18n.template}"
...
# Directory mode takes precedence over the single-file HOMETOWN_MOD_I18N_ANDROID_ICON:
# icons named <density>.png (ldpi/mdpi/hdpi/xhdpi/xxhdpi/xxxhdpi); a missing
# density falls back to the nearest available one.
HOMETOWN_MOD_I18N_ANDROID_ICON_DIR="${HOMETOWN_MOD_I18N_ANDROID_ICON_DIR:-$HOMETOWN_MOD_I18N_MOD_DIR/assets/icon/android}"
...
HOMETOWN_MOD_I18N_FETCH_SDK="${HOMETOWN_MOD_I18N_FETCH_SDK:-1}"
...
# (HOMETOWN_MOD_I18N_TOOLS_DIR) is only resolved when lib.sh is sourced above.
HOMETOWN_MOD_I18N_ANDROID_SDK_DIR="${HOMETOWN_MOD_I18N_ANDROID_SDK_DIR:-$HOMETOWN_MOD_I18N_TOOLS_DIR/android-sdk}"

read_mod_version() {
    version="$(sed -n 's/^[[:space:]]*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' \
        "$HOMETOWN_MOD_I18N_MOD_DIR/mod.json" | head -n 1)"
...
# HOMETOWN_MOD_I18N_ANDROID_SDK_DIR ($HOMETOWN_MOD_I18N_TOOLS_DIR/android-sdk by
# default — the shared tools dir outside the mod tree, same location the Windows
# launcher build_android.ps1 uses). Disable with HOMETOWN_MOD_I18N_FETCH_SDK=0.
...
install_android_sdk() {
    local sdk="$HOMETOWN_MOD_I18N_ANDROID_SDK_DIR"
    ...
    if [ -f "$sdkmanager_cmd" ] || [ -f "$sdkmanager_cmd.bat" ]; then
        ...
        archive="$HOMETOWN_MOD_I18N_MOD_DIR/.build/cache/commandlinetools-${os}-11076708_latest.zip"
        if [ ! -f "$archive" ]; then
...
ensure_android_sdk() {
    ...
    [ "$HOMETOWN_MOD_I18N_FETCH_SDK" = "1" ] || fail \
        "缺少完整的 Android SDK 且 HOMETOWN_MOD_I18N_FETCH_SDK=0；请设置 ANDROID_SDK_ROOT 指向含 API 34 和 NDK 25.2.9519653 的 SDK"

    # Point sdkmanager at the target dir before installing into it.
    export ANDROID_SDK_ROOT="$HOMETOWN_MOD_I18N_ANDROID_SDK_DIR"
    install_android_sdk
}
...
check_inputs() {
    ...
    # HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME/JAVA_HOME (wrong version is an error) or
    # a matching `java` on PATH, and auto-downloads a portable JDK 17 into
    # $HOMETOWN_MOD_I18N_TOOLS_DIR/jdk17 (shared tools dir) when the box has none.
    ensure_java 17
...
stage_android_icons() {
    ...
    for density in ldpi mdpi hdpi xhdpi xxhdpi xxxhdpi; do
        [ -f "$HOMETOWN_MOD_I18N_ANDROID_ICON_DIR/$density.png" ] && density_dir_has_icon=1
    done

    if [ -n "$HOMETOWN_MOD_I18N_ANDROID_ICON_DIR" ] && [ "$density_dir_has_icon" = "1" ]; then
        for density in ldpi mdpi hdpi xhdpi xxhdpi xxxhdpi; do
...
stage_android_source() {
    ...
    tar -cf - --exclude='*.git' \
        -C "$HOMETOWN_MOD_I18N_ANDROID_CACHE_DIR" . | tar -xf - -C "$stage_dir"
...
build_love_archive() {
    ...
    HOMETOWN_MOD_I18N_MOD_DIR="$HOMETOWN_MOD_I18N_MOD_DIR" \
        HOMETOWN_MOD_I18N_KRISTAL_SOURCE="${HOMETOWN_MOD_I18N_KRISTAL_SOURCE:-ask}" \
