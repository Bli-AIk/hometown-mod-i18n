...
#
# Source this after setting HOMETOWN_MOD_I18N_MOD_DIR.
...
HOMETOWN_MOD_I18N_LOVE="${HOMETOWN_MOD_I18N_LOVE:-}"
if [ -z "$HOMETOWN_MOD_I18N_LOVE" ]; then
    HOMETOWN_MOD_I18N_LOVE="$(resolve_love)" || {
...
# profile; explicit KRISTAL_ROOT / HOMETOWN_MOD_I18N_KRISTAL_DIR are only a
...
# Linux PHYSFS resolves the symlinks fine). Set HOMETOWN_MOD_I18N_TOOLS_DIR to
...
detect_kristal_root() {
    ...
    for candidate in "${KRISTAL_ROOT:-}" "${HOMETOWN_MOD_I18N_KRISTAL_DIR:-}"; do
        [ -n "$candidate" ] || continue
        # HOMETOWN_MOD_I18N_KRISTAL_DIR defaults to the mod-root clone
        # .build/Kristal — inside the mod, not a shared host. Skip it.
        [ "$candidate" = "$HOMETOWN_MOD_I18N_MOD_DIR/.build/Kristal" ] && continue
        [ -f "$candidate/main.lua" ] && { printf '%s\n' "$candidate"; return 0; }
    done
    return 1
}
HOMETOWN_MOD_I18N_TOOLS_DIR="${HOMETOWN_MOD_I18N_TOOLS_DIR:-}"
if [ -z "$HOMETOWN_MOD_I18N_TOOLS_DIR" ]; then
    _kr="$(detect_kristal_root || true)"
    HOMETOWN_MOD_I18N_TOOLS_DIR="${_kr:+$_kr/.tools}"
    : "${HOMETOWN_MOD_I18N_TOOLS_DIR:=$HOMETOWN_MOD_I18N_MOD_DIR/.tools}"
fi
...
# shells (CI never pops a window) and when HOMETOWN_MOD_I18N_NO_OPEN_DIR=1 (the
...
open_output_dir() {
    [ "${HOMETOWN_MOD_I18N_NO_OPEN_DIR:-0}" = "1" ] && return 0
    [ -t 1 ] || [ -t 2 ] || return 0
    local dir="$1"
...
run_helper() {
    ...
    HOMETOWN_MOD_I18N_HELPER_ARGS="$(win_path "$args_file")" \
        "$HOMETOWN_MOD_I18N_LOVE" "$HOMETOWN_MOD_I18N_MOD_DIR/build-helper"
    status=$?
    rm -f "$args_file"
    return $status
}
...
zip_dir() {
    ...
    # working directory first (CI passes e.g. HOMETOWN_MOD_I18N_OUTPUT_DIR=dist-win).
    case "$output" in
        /*) ;;
        *) output="$(pwd -P)/$output" ;;
    esac
    ...
    if command -v zip >/dev/null 2>&1; then
        ...
        if [ "${HOMETOWN_MOD_I18N_ZIP_FALLBACK_WARNED:-0}" != "1" ]; then
            warn "未找到系统 zip，改用 LÖVE 内置压缩助手（正常，构建继续）"
            HOMETOWN_MOD_I18N_ZIP_FALLBACK_WARNED=1
        fi
...
# HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME/JAVA_HOME and no usable `java` is on PATH,
# ensure_java downloads a portable Temurin JDK into $HOMETOWN_MOD_I18N_TOOLS_DIR/jdk<version>
# (the shared tools dir outside the mod tree) and exports JAVA_HOME/PATH. Disable
# the download with HOMETOWN_MOD_I18N_FETCH_JDK=0.
...
HOMETOWN_MOD_I18N_JDK_VERSION="${HOMETOWN_MOD_I18N_JDK_VERSION:-17}"
HOMETOWN_MOD_I18N_JDK_DIR="${HOMETOWN_MOD_I18N_JDK_DIR:-$HOMETOWN_MOD_I18N_TOOLS_DIR/jdk$HOMETOWN_MOD_I18N_JDK_VERSION}"
HOMETOWN_MOD_I18N_FETCH_JDK="${HOMETOWN_MOD_I18N_FETCH_JDK:-1}"
...
ensure_java() {
    local exact="${1:-}" version="${1:-$HOMETOWN_MOD_I18N_JDK_VERSION}" java_home major
    ...
    if [ -n "$java_home" ]; then
        ...
        if [ -n "$exact" ]; then
            ...
            [ "$major" = "$exact" ] || {
                fail "Java $exact required, found ${major:-unknown} at $java_home (set HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME to a JDK $exact)"
            }
        fi
    ...
    export JAVA_HOME="$HOMETOWN_MOD_I18N_JDK_DIR"
    export PATH="$JAVA_HOME/bin:$PATH"
}
...
# HOMETOWN_MOD_I18N_JDK_DIR (archive cached in .build/cache). Reuses an existing
# installation instead of re-downloading ~190 MB every build.
install_portable_jdk() {
    local version="$1" dest="$HOMETOWN_MOD_I18N_JDK_DIR"
    # Windows JDKs ship bin/java.exe, POSIX ones bin/java — accept either.
    if [ -x "$dest/bin/java" ] || [ -x "$dest/bin/java.exe" ]; then
        printf 'JDK %s 已存在: %s\n' "$version" "$dest" >&2
        return 0
    fi
    [ "$HOMETOWN_MOD_I18N_FETCH_JDK" = "1" ] || {
        fail "No JDK $version found and HOMETOWN_MOD_I18N_FETCH_JDK=0; install JDK $version or set HOMETOWN_MOD_I18N_ANDROID_JAVA_HOME"
    }
    ...
    cache="$HOMETOWN_MOD_I18N_MOD_DIR/.build/cache"
    mkdir -p "$cache"
