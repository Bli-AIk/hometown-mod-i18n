-- Build helper for hometown-mod-i18n's packaging scripts, run with the LÖVE
...
local GRADLE_REPLACEMENT = [[    def signingKeystore = System.getenv("HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEYSTORE")
    def hasCustomSigning = signingKeystore != null && !signingKeystore.isEmpty()
    def signingStorePassword = System.getenv("HOMETOWN_MOD_I18N_ANDROID_SIGNING_STORE_PASSWORD")
    def signingKeyAlias = System.getenv("HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_ALIAS")
    def signingKeyPassword = System.getenv("HOMETOWN_MOD_I18N_ANDROID_SIGNING_KEY_PASSWORD")
...
local function main()
    ...
    -- HOMETOWN_MOD_I18N_HELPER_ARGS (one argument per line).
    local args_file = os.getenv("HOMETOWN_MOD_I18N_HELPER_ARGS")
    if not args_file then
        fail("missing HOMETOWN_MOD_I18N_HELPER_ARGS (run via build-helper/lib.sh)")
    end
    local args = {}
