...
if not defined LATEST (
    echo [hometown-mod-i18n] Could not check for updates, using cached build.
    goto run-cached
)
...
:need-download
echo [hometown-mod-i18n] Downloading kristal-debug-tools GUI (latest release)...
...
if not defined PREV (
    echo [hometown-mod-i18n] The latest release is not ready and no previous release was found. Try again later.
    exit /b 1
)
if defined LATEST (
    echo [hometown-mod-i18n] The latest release ^(%LATEST%^) is still being built; falling back to previous release %PREV%.
) else (
    echo [hometown-mod-i18n] The latest release is still being built; falling back to previous release %PREV%.
)
...
echo [hometown-mod-i18n] Previous release %PREV% is already downloaded.
set "USE_CACHED="
set /p USE_CACHED="[hometown-mod-i18n] Use the cached previous release %PREV%? [y/N] "
if /i "%USE_CACHED%"=="y" goto run-cached
if /i "%USE_CACHED%"=="yes" goto run-cached
echo [hometown-mod-i18n] Not using the cached release. Try again later once the latest release is ready.
exit /b 1

:download-prev
echo [hometown-mod-i18n] Downloading previous release %PREV%...
set "DL_BASE=https://github.com/Bli-AIk/kristal-debug-tools-gui/releases/download/%PREV%/"
call :download-assets "%DL_BASE%"
if errorlevel 1 (
    echo [hometown-mod-i18n] Could not download the latest or previous release. Check your network or build locally.
    exit /b 1
)
