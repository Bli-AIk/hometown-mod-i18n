...
$environmentNames = @('HOMETOWN_MOD_I18N_TOOLS_DIR', 'HOMETOWN_MOD_I18N_KRISTAL_DIR', 'KRISTAL_ROOT')
$previous = @{}
...
try {
    $override = Join-Path ([System.IO.Path]::GetTempPath()) ("hometown-mod-i18n-tools-" + [guid]::NewGuid().ToString('N'))
    $env:HOMETOWN_MOD_I18N_TOOLS_DIR = $override
    ...
    Remove-Item Env:HOMETOWN_MOD_I18N_TOOLS_DIR -ErrorAction SilentlyContinue
    Remove-Item Env:HOMETOWN_MOD_I18N_KRISTAL_DIR -ErrorAction SilentlyContinue
    ...
    if ($love) {
        ...
        $zipRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("hometown-mod-i18n-helper-zip-" + [guid]::NewGuid().ToString('N'))
